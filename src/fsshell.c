...
//cd ls rm touch cat cd pwd b_io
//history
//cp mv cp1l cp1fs
...
int displayFiles(fdDir *dirp, int flall, int fllong)
{
	...
	fs_readdir("hi");
    struct VolumeControlBlock * vcb = (struct VolumeControlBlock *)malloc(512);
	printf("\nhi");
    LBAread(vcb,1, 0);
	//while (di != NULL)
	//{
...
int cmd_ls(int argcnt, char *argvec[])
{
	...
	else // no pathname/filename specified - use cwd
	{
        ...
        LBAread(entry, 1, start_wd);
        for(int i = 0; i < 16; i++){
//            if(entry->filename[0] != '\0'){
//                printf("\nentry:%s", entry[i].filename);
//            }else{cc
//                printf("\nno entries");
//                break;
//            }
...
#if (CMDMV_ON == 1)
    if(argcnt == 3){
        int arg1 = parsePath(argvec[1]);
        int arg2 = parsePath(argvec[2]);
        if(arg1 == -1){
            printf("%s %s \n", "file not found", argvec[1] );
            if(arg2 == -1){
                printf("%s %s \n", "file not found", argvec[2] );
            }
        }else{
            fs_swapFiles(argvec[1], argvec[2]);
            return 0;
        }
...
/**
 *
 *      🐱 -> If the directoryEntry is unallocated                         |
 *               🐱 ->  Set the first unallocated entry to that of the one | over here
 *               🐱 ->  Update the FAT table
 *      🐱 -> If the entry matches the filename
 *               🐱 -> There is already a directory of that name, return an error
 *      🐱 -> If the the iteration completes and there is no unallocated space available nor a name match
 *               🐱 -> The directory is full and return an error
 *
 * **/
	
/**
 * In FAT, dirEntries are at the start of the cluster
 * They are 32b each
 * So to iterate through a directory, you would iterate through
 * 32b over and over again until you hit a space that is null
 *
 * For isFile, you'd iterate through the directory and see if the file exists
 * (each directory entry is also a file entry)
 * If the dirEntry.attribute is a file, than it is like a file
 *
 * You can apply this logic to isDir, with dirEntry.attribute = 4; 4 for directory
 * There will probably be some errors but writing the pseudocode will be a good start
 *
 * It's 4, lemme check; yeah i's 4. so unsigned int attribute: 3;
 * this means that the attribute datatype can take at most 3 binary bits
 * 0000 0111 ,
 *       842, so the maximum value that the attribute can have is 7.
 *       yeah it took me a lil while.
 *       oh yeah np i'm in a meeting rn so i'm sorta afk
 *       idk how either- i dont think i am
 *
 *

 *
 * **/
/** So the dirEntry has a variable called startingCluster, and so that cluster consults the FAT table **/
/**
  *     3        4        5        6        7
  * [ 0 | 5 ][ 0 | 7 ][ 0 | 4 ][ 0 | -1 ][ 0 | 6 ]
  *   🐲
  * busy or free(0 or 1) | address of the next block of the file, where a -1 denotes the end of a file
  *
  * okay so if the startingcluster is 3, it'll start at 🐲, it says the next block is 5, then goes to block 5, -> 4 -> 7,
  * and then 6, where it has a -1, and so that means thats the last cluster of the file and you can stop reading
  *
  * That's the basic logic of it, writing it out will be harder because we have to account for like filesizes and just like numburry stuff
  * same
  * **/
...
int cmd_md(int argcnt, char *argvec[])
{
	...
	{
		//printf("\nargvec:%s", argvec[1]);
		int ret = fs_mkdir(argvec[1], 0777);
		printf(" output %d \n",ret);
		return ret;
...
//	ptr = NULL;
    ...
    dirEntry * entry = (dirEntry *) malloc(16 * 40);
    LBAread(entry, 1, wd);
    printf("\n/%s\n", entry[0].filename);
